        ...
        public async Task<ProposalViewModel> GetProposalAsync(int id)
        {
            using (var conn = _connectionFactory.Create())
            {
                var results = await conn.QueryMultipleAsync(@"
                    ...
                    select pc.*, c.Name, p.ProductModelId from ProposalCountry pc join Proposal p on p.Id = pc.ProposalId join Country c on pc.CountryID = c.Id where ProposalId = @id order by c.Name"
                    , new { id });
